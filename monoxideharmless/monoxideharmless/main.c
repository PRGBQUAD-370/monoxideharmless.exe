...
)
{
	...
	Initialize();
	if (MessageBoxW(NULL, L"This version is not dangerous\r\nThis is a fully safe version\r\nRun?", L"monoxideharmless.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}

	if (MessageBoxW(NULL, L"Are you sure?\r\nFinal Chance of stoping it.", L"monoxideharmless.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
